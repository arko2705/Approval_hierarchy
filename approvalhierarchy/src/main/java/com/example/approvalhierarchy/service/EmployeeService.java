...
public class EmployeeService {
    ...
    public void deleteEmployee(Long id) {
        employeeRepository.deleteById(id);// built in jpa repository method,we didnt have to write it ourselves.
    }
     ...
     * Builds the tree in-memory within a transaction to avoid lazy-loading issues.
     */
    @Transactional(readOnly = true)
    public List<Employee> getOrganizationalHierarchy() {
        // Load ALL employees in one query while the JPA session is open
        List<Employee> allEmployees = employeeRepository.findAll();

        // Build a map of id -> employee for quick lookup
        Map<Long, Employee> employeeMap = new HashMap<>();
        for (Employee e : allEmployees) {
            e.setSubordinates(new ArrayList<>());
            employeeMap.put(e.getId(), e);
        }

        // Wire up subordinates manually
        List<Employee> roots = new ArrayList<>();
        for (Employee e : allEmployees) {
            if (e.getManager() == null) {
                roots.add(e);
            } else {
                Employee parent = employeeMap.get(e.getManager().getId());
                if (parent != null) {
                    parent.getSubordinates().add(e);
                }
            }
        }
